...
#                 self.status_label.config(text="오류 발생: 매크로를 중지합니다.")
#                 self.is_running = False
#                 break
...
def save_to_db(excel_file):
    try:
        ...
        if df.shape[1] == 4:
            df.columns = ['region', 'registration_number', 'store_name', 'detailed_area']
            df['open'] = ''

        for index, row in df.iterrows():
            cursor.execute("SELECT * FROM business_data WHERE region=? AND store_name=?", (row['region'], row['store_name']))
            existing_data = cursor.fetchone()
            if existing_data is None:
                cursor.execute("INSERT INTO business_data (region, registration_number, store_name, detailed_area, open) VALUES (?, ?, ?, ?, ?)",
                               (row['region'], row['registration_number'], row['store_name'], row['detailed_area'], row['open']))
...
class MacroApp:
    ...
    def start_macro(self):
        self.is_running = True
        self.update_status("매크로가 실행 중입니다...")
        threading.Thread(target=self.run_macro).start()

    def stop_macro(self):
        self.is_running = False
        self.update_status("매크로가 중지되었습니다.")
        self.save_progress_to_excel()
    ...
    def save_progress_to_excel(self):
        try:
            ...
            print(f"현재 진행 상태가 '{self.excel_file_name}' 파일에 저장되었습니다.")
        except Exception as e:
            print(f"진행 상태 저장 중 오류 발생: {e}")
            traceback.print_exc()
    ...
    def run_macro(self):
        ...
        for index in range(self.current_index, len(self.data)):
            ...
            try:
                print(f"현재 처리 중인 데이터: ID={id}, 지역={region}, 가게 이름={store_name}")

                # region 입력
                pyautogui.moveTo(757, 219, duration=1)
                pyautogui.click()
                time.sleep(1)
                pyautogui.typewrite(region, interval=0.1)
                pyautogui.press('enter')
                time.sleep(1)

                # store_name 입력
                pyautogui.moveTo(1406, 915, duration=1)
                pyautogui.click()
                time.sleep(1)
                pyautogui.typewrite(store_name, interval=0.1)
                pyautogui.press('enter')
                time.sleep(1)

                # 이미지 인식
                image_path_1 = 'images/1.png'
                image_path_2 = 'images/2.png'

                try:
                    location_1 = pyautogui.locateOnScreen(image_path_1, confidence=0.6)
                    location_2 = pyautogui.locateOnScreen(image_path_2, confidence=0.6)
                ...
                if location_1 or location_2:
                    cursor.execute("UPDATE business_data SET open = 'T' WHERE id = ?", (id,))
                    self.update_status("이미지 발견! 다음 작업으로 진행합니다.")
                else:
                    cursor.execute("UPDATE business_data SET open = 'F' WHERE id = ?", (id,))
                    self.update_status("이미지 없음. 다음 작업으로 진행합니다.")
                conn.commit()
                conn.close()
        ...
        if self.is_running:
            self.current_index = 0
            self.update_status("매크로가 완료되었습니다.")
